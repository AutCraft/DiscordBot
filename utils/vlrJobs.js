...
function startVlrJobs(Client) {
    ...
    setInterval(async () => {
        try {
            let notifyDb = JSON.parse(fs.readFileSync(notifyDbPath, 'utf8'));
            let followDb;
            try {
            ...
            const res = await vlrApi.get('/match?q=upcoming').catch(() => null);
            ...
            for (const match of upcoming) {
                // If match is within 15 minutes (15 * 60 * 1000 = 900000ms)
                if (!match.unix_timestamp) continue;
                
                const ms = String(match.unix_timestamp).length <= 10 && !isNaN(Number(match.unix_timestamp)) ? Number(match.unix_timestamp) * 1000 : match.unix_timestamp;
                    ...
                    if (!notifyDb.notifiedMatches.includes(matchId)) {
                        ...
                        if (notifyDb.enabled && notifyDb.channelId) {
                            ...
                            if (channel) {
                                ...
                                const ascii = buildAsciiMatch(match.team1, match.team2, 'vs', match.match_event, `เริ่มใน ~${Math.round(timeDiff / 60000)} นาที`);
                        ...
                        const team1Id = lookup.teams[match.team1.toLowerCase()];
                        const team2Id = lookup.teams[match.team2.toLowerCase()];

                        for (const [userId, followedTeams] of Object.entries(followDb)) {
                            if ((team1Id && followedTeams.includes(team1Id)) || (team2Id && followedTeams.includes(team2Id))) {
